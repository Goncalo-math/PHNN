...
from utilis import L2_loss, beta, funct, integrate
    ...
    if(args.integrator == 'Euler'):
      for i in range(x.shape[0]):
        ...
        if(i%num_steps) == 0:
          ...
          dqdt = grad(hamilt.sum(), p, create_graph=True)[0]

          p_next = x[i-1,1] + dpdt*h
          q_next = x[i-1,0] + dqdt*h
...
if __name__ == "__main__":
  ...
  ## Poisson-Hamiltonian Integrator Neural Network
  args = get_args()
