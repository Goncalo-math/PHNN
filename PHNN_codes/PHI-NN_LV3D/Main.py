    ...
    if(args.integrator == 'Euler'):
      for i in range(x.shape[0]):
        ...
        if(i%90) == 0:
          ...
          dqdt = q*p*dhdp + q*z*dhdz
          dpdt = p*z*dhdz - q*p*dhdq
          dzdt = -q*z*dhdq - p*z*dhdp

          q_next = x[i-1,0] + dqdt*1/15
          p_next = x[i-1,1] + dpdt*1/15
          z_next = x[i-1,2] + dzdt*1/15
    ...
    if(args.integrator == 'PHI'):
      for i in range(x.shape[0]):
        ...
        if(i%90) == 0:
          ...
          q_next, p_next, z_next = beta(y,1/15*dhdy)
...
## Integrate the model
  ...
  ax.plot3D(result0[:,0].detach().numpy(), result0[:,1].detach().numpy(), result0[:,2].detach().numpy(),'--g')
  ax.set(xlabel='$x^1$', ylabel='$x^2$', zlabel='$x^3$')
  ax.legend(['Real', 'PHI-PHI'], loc='upper right', fontsize = 15)
  plt.show()
