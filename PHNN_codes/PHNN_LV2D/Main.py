...
if __name__ == "__main__":
  ...
  #ax.set_title('Lotka-Volterra with PHNN and real solution')
  ax.plot(real[:,0], real[:,1],'-k')
  ...
  pnn_ivp1 = integrate_model(model, t_span, y1, **kwargs)

  result = pnn_ivp['y'].T
  result1 = pnn_ivp1['y'].T

  ax.plot(result[:,0], result[:,1],'--r')
  ax.plot(result1[:,0], result1[:,1],'--r')
  ax.set_xlabel('q')
  ax.set_ylabel('p')
  ax.legend(['Real', '_hide', 'PHNN'], loc='upper right', fontsize = 15)
  plt.show()
