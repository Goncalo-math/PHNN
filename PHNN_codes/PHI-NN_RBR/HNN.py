import torch
import numpy as np
from utilis import choose_nonlinearity, rk4
from data import poisson_fn
...
class MLP(torch.nn.Module):
  ...
  def __init__(self, input_dim, hidden_dim, output_dim, nonlinearity='tanh'):
    ...
    #Inicializar com matriz ortogonal
    ...
    #Definir o passo em frnte da NN
  def forward(self, x, separate_fields=False):
    h = self.nonlinearity( self.linear1(x) ) # h = tanh(L1*x + b1)
    h = self.nonlinearity( self.linear2(h) ) # h = tanh(L2*h + b2)
    h = self.nonlinearity( self.linear3(h) )
    return self.linear4(h)                    # return L3*h
...
class HNN(torch.nn.Module):
    ...
    def get_solenoidal(self, x, dF):
        solenoidal_field = torch.zeros_like(x)

        for l in range(x.size(0)):
            solenoidal_field[l,:] = torch.tensor(poisson_fn(x[l,:].detach().numpy()), requires_grad=True, dtype=torch.float32) @ dF[l,:]
    ...
    def time_derivative(self, x, t=None, separate_fields=False):
        ...
        if self.field_type != 'conservative':
            ...
            solenoidal_field = self.get_solenoidal(x, dF2)
    ...
    def get_solenoidal_fn(self, x, dF):
        solenoidal_field = torch.zeros_like(x)

        solenoidal_field = torch.tensor(poisson_fn(x.detach().numpy()), requires_grad=True, dtype=torch.float32) @ dF

        return solenoidal_field


    def time_derivative_fn(self, x, t=None, separate_fields=False):
            '''NEURAL ODE-STLE VECTOR FIELD'''
            if self.baseline:
                return self.differentiable_model(x)

            '''NEURAL HAMILTONIAN-STLE VECTOR FIELD'''
            F1, F2 = self.forward_fn(x) # traditional forward pass
            ...
            if self.field_type != 'conservative':
                dF2 = torch.autograd.grad(F2.sum(), x, create_graph=True)[0] # gradients for solenoidal field
                # solenoidal_field = dF2 @ self.M
                solenoidal_field = self.get_solenoidal_fn(x, dF2)
